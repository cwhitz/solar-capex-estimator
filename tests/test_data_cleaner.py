...
class TestDataCleanerInit:
    ...
    def test_init_with_default_parameters(self):
        ...
        assert cleaner.na_drop_thresholds == {'string_columns': 0.10, 'numeric_columns': 0.50}
...
class TestDataCleanerTargetCleaning:
    ...
    def test_clean_by_target_removes_missing_values(self, sample_raw_data_with_missing):
        ...
        result = cleaner._clean_by_target(cleaner.df, target_col='total_installed_price')
...
class TestDataCleanerColumnDropping:
    ...
    def test_drop_high_na_columns_string_threshold(self):
        ...
        cleaner = DataCleaner(na_drop_thresholds={'string_columns': 0.5, 'numeric_columns': 0.8})
        df = pd.DataFrame({
            'col1': ['a', 'b', 'c', 'd', 'e'],
    ...
    def test_drop_high_na_columns_numeric_threshold(self):
        ...
        cleaner = DataCleaner(na_drop_thresholds={'string_columns': 0.5, 'numeric_columns': 0.5})
        df = pd.DataFrame({
    ...
    def test_drop_high_cardinality_columns(self):
        ...
        df = pd.DataFrame({
            ...
            'numeric': [1, 2, 3, 4, 5]  # Should not be dropped even if high cardinality
        })
        cleaner.load_data(df)
